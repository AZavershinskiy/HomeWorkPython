...
def work(path, dict_fieldnames, fieldnames):
    while True:
...
def add_new_user(path, fieldnames):
    user_first_name = view.data_request('Введите имя: ')
    user_second_name = view.data_request('Введите фамилию: ')
    user_phone = view.data_request('Введите телефон: ')
    user_note = view.data_request('Введите примечание: ')
    user_list = db.read_file_csv(path)
    check_file = os.path.isfile(path)
    new_user_dict = contact_handler.user_dictionary(
        check_file, user_list, user_first_name, user_second_name, user_phone, user_note)
    db.adding_data_to_the_database_csv(
        new_user_dict, check_file, path, fieldnames)
...
def edit_user(path, dict_fieldnames, fieldnames):
    ...
    user_list = contact_handler.edit_user(
        users, id_user, what_to_edit, dict_fieldnames)
    db.overwriting_file_csv(user_list, path, fieldnames)


def search_user(path, dict_fieldnames, fieldnames):
    user = view.data_request('Введите идентификатор пользователя: ')
    search_field = int(view.data_request(
        'Введите поле для поиска (1 - Имя, 2 - Фамилия, 3 - Телефон, 4 - Примечание): '))
    users = db.read_file_csv(path)
...
def delete_user(path, dict_fieldnames, fieldnames):
    user = view.data_request('Введите идентификатор пользователя: ')
    search_field = int(view.data_request(
        'Введите поле для поиска (1 - Имя, 2 - Фамилия, 3 - Телефон, 4 - Примечание): '))
    users = db.read_file_csv(path)
...
def print_all_users(path, fieldnames):
    users = db.read_file_csv(path)
    view.print_user(users, fieldnames)


def import_file(path, dict_fieldnames, fieldnames):
    ...
    new_user_list_dict = contact_handler.data_preparation_user_in_csv(
        user_list_txt, dict_fieldnames)
    check_file = os.path.isfile(path)
...
def export_file(path, fieldnames):
    user_list = contact_handler.data_preparation_user_in_txt(
        db.read_file_csv(path))
