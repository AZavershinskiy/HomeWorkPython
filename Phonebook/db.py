...
def overwriting_file_csv(user_list, path, fieldnames):
    with open(path, 'w', newline='') as csvfile:
        name_dict_writer = csv.DictWriter(
            csvfile, fieldnames=fieldnames, delimiter=';', dialect='excel-tab')
        name_dict_writer.writeheader()
        for i in user_list:
            name_dict_writer.writerow(i)


def adding_data_to_the_database_csv(user_dict, check_file, path, fieldnames):
